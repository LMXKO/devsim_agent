...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fake-backend", action="store_true", help="Run the interface-only fake backend in the current project directory.")
...
def main() -> None:
    ...
    try:
        ...
        if not args.project_path:
            raise ValueError("--project is required unless --all-fixtures or --fake-backend is set")
        ...
        print(json.dumps({"tool_name": "sentaurus_contract", "status": "failed", "failure_reason": str(exc)}, indent=2, ensure_ascii=False))
        raise SystemExit(2)
