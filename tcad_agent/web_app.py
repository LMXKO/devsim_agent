...
def render_legacy_app_html() -> str:
    preset_json = json.dumps(PRESET_GOALS, ensure_ascii=False)
    return f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>TCAD Mission Workbench</title>
  <style>
    :root {{
      color: #17202a;
      background: #f5f7f8;
    ...
    * {{ box-sizing: border-box; }}
    ...
    aside {{ background: #111827; color: #f9fafb; padding: 18px 16px; display: flex; flex-direction: column; gap: 18px; }}
    .brand {{ display: flex; align-items: center; gap: 10px; min-height: 36px; }}
    .mark {{ width: 28px; height: 28px; border-radius: 6px; background: #2dd4bf; box-shadow: inset 0 -8px 0 rgba(0,0,0,.16); }}
    ...
    .nav a.active {{ color: #111827; background: #f9fafb; }}
    .capabilities {{ display: grid; gap: 8px; margin-top: auto; }}
    .capability {{ border: 1px solid #374151; border-radius: 6px; padding: 8px; }}
    .capability strong {{ display: block; font-size: 12px; }}
    .capability span {{ color: #cbd5e1; font-size: 12px; line-height: 1.35; }}
    main {{ display: grid; grid-template-rows: auto 1fr; min-width: 0; }}
    header {{ background: #ffffff; border-bottom: 1px solid #d9dee5; padding: 12px 18px; display: flex; gap: 12px; justify-content: space-between; align-items: center; }}
    .header-title {{ display: grid; gap: 2px; }}
    .header-title strong {{ font-size: 15px; }}
    .header-title span {{ color: #617080; font-size: 12px; }}
    .toolbar {{ display: flex; flex-wrap: wrap; gap: 8px; align-items: center; }}
    .btn {{ border: 1px solid #c7d0da; background: #ffffff; color: #17202a; border-radius: 6px; padding: 7px 10px; min-height: 34px; cursor: pointer; }}
    .btn.primary {{ background: #0f766e; border-color: #0f766e; color: #ffffff; }}
    .btn.danger {{ border-color: #fca5a5; color: #991b1b; }}
    .btn:disabled {{ opacity: .55; cursor: default; }}
    .workspace {{ padding: 16px 18px 28px; display: grid; grid-template-columns: minmax(360px, 520px) minmax(0, 1fr); gap: 16px; min-width: 0; }}
    section {{ min-width: 0; }}
    .panel {{ background: #ffffff; border: 1px solid #d9dee5; border-radius: 8px; overflow: hidden; }}
    .panel h2 {{ margin: 0; padding: 12px 14px; border-bottom: 1px solid #e5e9ee; font-size: 14px; letter-spacing: 0; }}
    .panel-body {{ padding: 14px; display: grid; gap: 12px; }}
    label {{ display: grid; gap: 6px; color: #374151; font-size: 12px; }}
    ...
    .checkline {{ display: flex; align-items: center; gap: 8px; color: #263241; font-size: 13px; }}
    .checkline input {{ width: 16px; height: 16px; min-height: 16px; }}
    .presets {{ display: grid; gap: 7px; }}
    .preset {{ text-align: left; border: 1px solid #d9dee5; background: #f8fafc; border-radius: 6px; padding: 8px; cursor: pointer; line-height: 1.35; }}
    .metrics {{ display: grid; grid-template-columns: repeat(4, minmax(120px, 1fr)); gap: 10px; }}
    .metric {{ background: #ffffff; border: 1px solid #d9dee5; border-radius: 8px; padding: 10px; min-height: 64px; }}
    .metric span {{ color: #617080; font-size: 12px; }}
    .metric strong {{ display: block; font-size: 20px; margin-top: 3px; }}
    .stack {{ display: grid; gap: 16px; min-width: 0; }}
    .table-wrap {{ overflow: auto; max-height: 360px; }}
    table {{ border-collapse: collapse; width: 100%; font-size: 12px; }}
    th, td {{ text-align: left; padding: 8px; border-bottom: 1px solid #edf0f3; vertical-align: top; }}
    th {{ position: sticky; top: 0; background: #f8fafc; color: #475569; }}
    .status {{ display: inline-flex; align-items: center; min-height: 22px; padding: 2px 7px; border-radius: 999px; background: #e5e7eb; color: #374151; }}
    .status.completed, .status.passed {{ background: #dcfce7; color: #14532d; }}
    .status.running {{ background: #dbeafe; color: #1e3a8a; }}
    .status.failed {{ background: #fee2e2; color: #7f1d1d; }}
    .status.queued, .status.planned {{ background: #fef3c7; color: #78350f; }}
    .row-actions {{ display: flex; gap: 6px; flex-wrap: wrap; }}
    .mini {{ min-height: 28px; padding: 4px 7px; font-size: 12px; }}
    .log {{ background: #0b1220; color: #d8e2f0; border-radius: 6px; padding: 10px; min-height: 80px; max-height: 160px; overflow: auto; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 12px; white-space: pre-wrap; }}
...
<body>
  <div class="shell">
    <aside>
      <div class="brand"><div class="mark"></div><h1>TCAD Mission</h1></div>
      <nav class="nav">
    ...
    <main>
      <header>
        <div class="header-title">
          <strong>Autonomous TCAD Workbench</strong>
          <span id="headerMeta">Loading state</span>
        </div>
        <div class="toolbar">
          <button class="btn" id="refreshBtn" type="button">Refresh</button>
          <button class="btn" id="llmBtn" type="button">LLM Check</button>
          <button class="btn primary" id="startWorkerBtn" type="button">Start Worker</button>
          <button class="btn danger" id="stopWorkerBtn" type="button">Stop</button>
        </div>
      </header>
      <div class="workspace">
        <section class="stack">
          <div class="panel" id="mission">
            <h2>Mission Input</h2>
            <form class="panel-body" id="missionForm">
              <label>Goal
                <textarea id="goalText" name="goal_text" required></textarea>
              </label>
              <div class="grid-2">
                <label>Priority
                  <input id="priority" type="number" value="10">
                </label>
                <label>Max Cycles
                  <input id="maxCycles" type="number" min="1" value="12">
                </label>
              </div>
              <div class="switches">
                <label class="checkline"><input id="execute" type="checkbox" checked> Execute</label>
                <label class="checkline"><input id="useLlm" type="checkbox" checked> LLM decomposer</label>
                <label class="checkline"><input id="allowFallback" type="checkbox" checked> Allow deterministic fallback</label>
              </div>
              <button class="btn primary" type="submit">Submit Mission</button>
            </form>
          </div>
          <div class="panel">
            <h2>Presets</h2>
            <div class="panel-body presets" id="presets"></div>
          </div>
          <div class="panel">
            <h2>Event Log</h2>
        ...
        <section class="stack">
          <div class="metrics">
            <div class="metric"><span>Queue</span><strong id="metricQueue">0</strong></div>
            <div class="metric"><span>Experiments</span><strong id="metricExperiments">0</strong></div>
            <div class="metric"><span>Benchmarks</span><strong id="metricBenchmarks">0</strong></div>
            <div class="metric"><span>Worker</span><strong id="metricWorker">off</strong></div>
          </div>
          ...
          <div class="panel" id="experiments">
            ...
            <div class="table-wrap"><table id="experimentTable"></table></div>
          </div>
          <div class="panel" id="quality">
            <h2>LLM and Quality</h2>
  ...
  <script>
    ...
    function statusPill(status) {{
      ...
      return `<span class="status ${{esc(text)}}">${{esc(text)}}</span>`;
    }}
    ...
    function renderCapabilities() {{
      document.getElementById('capabilities').innerHTML = capabilities.map(item =>
        `<div class="capability"><strong>${{esc(item.name)}}</strong><span>${{esc(item.scope)}}</span></div>`
      ).join('');
    }}

    function renderPresets() {{
      const el = document.getElementById('presets');
      el.innerHTML = presets.map(text => `<button class="preset" type="button">${{esc(text)}}</button>`).join('');
      [...el.querySelectorAll('.preset')].forEach((button, index) => {{
        button.addEventListener('click', () => document.getElementById('goalText').value = presets[index]);
      }});
      document.getElementById('goalText').value = presets[0] || '';
    }}
    ...
    function renderQueue(items) {{
      const rows = (items || []).map(item => `<tr>
        <td>${{esc(item.queue_id)}}</td>
        <td>${{statusPill(item.status)}}</td>
        <td>${{esc(item.tool_name)}}</td>
        <td>${{esc(item.priority)}}</td>
        <td>${{esc(item.attempts)}}/${{esc(item.max_attempts)}}</td>
        <td>${{esc(item.result_state_path || item.failure_reason || '')}}</td>
        <td>${{actionButtons(item)}}</td>
      </tr>`).join('');
      document.getElementById('queueTable').innerHTML =
        '<thead><tr><th>ID</th><th>Status</th><th>Tool</th><th>Priority</th><th>Attempts</th><th>Result</th><th>Actions</th></tr></thead><tbody>' +
        (rows || '<tr><td colspan="7">No queue items.</td></tr>') + '</tbody>';
    }}

    function renderExperiments(items) {{
      const rows = (items || []).map(item => `<tr>
        <td>${{esc(item.experiment_id)}}</td>
        <td>${{esc(item.kind)}}</td>
        <td>${{statusPill(item.status)}}</td>
        <td>${{statusPill(item.quality_status)}}</td>
        <td>${{esc(item.objective_value)}}</td>
        <td>${{esc(item.state_path)}}</td>
      </tr>`).join('');
      document.getElementById('experimentTable').innerHTML =
        '<thead><tr><th>ID</th><th>Kind</th><th>Status</th><th>Quality</th><th>Objective</th><th>State</th></tr></thead><tbody>' +
        (rows || '<tr><td colspan="6">No indexed experiments.</td></tr>') + '</tbody>';
    }}
    ...
    function renderQuality(state) {{
      ...
      const rows = [
        ['LLM status', statusPill(llm.status), llm.model || '', llm.base_url || ''],
        ['LLM live', llm.checked_live ? 'checked' : 'not checked', llm.latency_seconds || '', llm.failure_reason || llm.response_excerpt || ''],
        ['Worker', worker.running ? statusPill('running') : statusPill('stopped'), worker.owner || '', worker.failure_reason || worker.stop_file || ''],
        ['Queue DB', '', '', state.queue_db_path || ''],
      ].map(row => `<tr><td>${{esc(row[0])}}</td><td>${{row[1]}}</td><td>${{esc(row[2])}}</td><td>${{esc(row[3])}}</td></tr>`).join('');
      document.getElementById('qualityTable').innerHTML =
        '<thead><tr><th>Item</th><th>Status</th><th>Value</th><th>Detail</th></tr></thead><tbody>' + rows + '</tbody>';
    }}
    ...
    async function refresh() {{
      ...
      document.getElementById('metricBenchmarks').textContent = counts.benchmarks || 0;
      document.getElementById('metricWorker').textContent = state.worker_status && state.worker_status.running ? 'on' : 'off';
      document.getElementById('headerMeta').textContent = `${{state.generated_at || ''}}  ${{state.root || ''}}`;
      renderQueue(state.queue_items);
      renderExperiments(state.experiment_records);
      renderQuality(state);
    }}
    ...
    document.getElementById('missionForm').addEventListener('submit', async event => {{
      ...
      try {{
        const item = await api('/api/missions', {{method: 'POST', body: JSON.stringify(payload)}});
        log(`queued ${{item.queue_id}}`);
        await refresh();
      }} catch (error) {{
    ...
    document.addEventListener('click', async event => {{
      ...
      try {{
        await api(`/api/items/${{encodeURIComponent(target.dataset.id)}}/${{target.dataset.action}}`, {{method: 'POST', body: '{{}}'}});
        log(`${{target.dataset.action}} ${{target.dataset.id}}`);
        await refresh();
      }} catch (error) {{
    ...
    document.getElementById('refreshBtn').addEventListener('click', () => refresh().catch(error => log(error.message)));
    ...
    document.getElementById('stopWorkerBtn').addEventListener('click', async () => {{
      ...
      log('worker stop requested');
      await refresh();
    }});
    document.getElementById('llmBtn').addEventListener('click', async () => {{
      log('checking LLM');
      const result = await api('/api/llm/check', {{method: 'POST', body: '{{}}'}});
      log(`LLM ${{result.status}} ${{result.failure_reason || result.response_excerpt || ''}}`);
      await refresh();
    }});
