...
def industrial_runner_descriptors() -> list[IndustrialRunnerDescriptor]:
    return [
        ...
        IndustrialRunnerDescriptor(
            runner_id="gan_hemt_sentaurus_external",
            template_id="gan_hemt_id_bv",
            display_name="GaN HEMT Sentaurus External Runner",
            tool_name="industrial_external_tcad_runner",
            default_request={"template_id": "gan_hemt_id_bv", "simulator": "sentaurus"},
            command="python3.11 -m tcad_agent.tools.industrial_external_runner --template-id gan_hemt_id_bv --simulator sentaurus --project <USER_OWNED_PROJECT>",
            maturity=RunnerMaturity.REAL_EXTERNAL,
            dimensionality="external_2d_or_3d",
            solver_backend="sentaurus_external_workspace",
            solver_invoked=True,
            signoff_level="external_workspace_tcad_evidence",
            metrics=["threshold_voltage_v", "on_current_a", "breakdown_voltage_v", "current_collapse_proxy"],
            artifacts=["state.json", "runner_contract.json", "sentaurus_state.json", "solver_logs", "curve_csv", "deck_ir_artifacts"],
            capability_boundary="Runs only against a user-owned Sentaurus workspace/profile outside the repository; repository stores contracts, diffs, logs, and extracted evidence only.",
            signoff_gaps=["user_owned_project_required", "license_profile_required", "trap_self_heating_calibration", "golden_or_measured_correlation"],
            public_source_ids=["sentaurus_quasistationary_training", "genius_tcad_open", "gts_tutorial_catalog"],
        ),
        IndustrialRunnerDescriptor(
        ...
        IndustrialRunnerDescriptor(
            runner_id="sic_power_diode_sentaurus_external",
            template_id="sic_power_diode_bv_leakage",
            display_name="SiC Power Diode Sentaurus External Runner",
            tool_name="industrial_external_tcad_runner",
            default_request={"template_id": "sic_power_diode_bv_leakage", "simulator": "sentaurus"},
            command="python3.11 -m tcad_agent.tools.industrial_external_runner --template-id sic_power_diode_bv_leakage --simulator sentaurus --project <USER_OWNED_PROJECT>",
            maturity=RunnerMaturity.REAL_EXTERNAL,
            dimensionality="external_2d_or_3d",
            solver_backend="sentaurus_external_workspace",
            solver_invoked=True,
            ...
            artifacts=["state.json", "runner_contract.json", "sentaurus_state.json", "solver_logs", "curve_csv", "deck_ir_artifacts"],
            capability_boundary="Runs only against a user-owned Sentaurus SiC/JBS/SBD workspace/profile outside the repository.",
            signoff_gaps=["user_owned_project_required", "license_profile_required", "wide_bandgap_model_calibration", "mesh_convergence", "golden_or_measured_correlation"],
            public_source_ids=["sentaurus_quasistationary_training", "devsim_core_examples", "genius_tcad_open"],
        ),
        IndustrialRunnerDescriptor(
        ...
        IndustrialRunnerDescriptor(
            runner_id="igbt_turnoff_sentaurus_external",
            template_id="igbt_output_turnoff",
            display_name="IGBT Turn-off Sentaurus External Runner",
            tool_name="industrial_external_tcad_runner",
            default_request={"template_id": "igbt_output_turnoff", "simulator": "sentaurus"},
            command="python3.11 -m tcad_agent.tools.industrial_external_runner --template-id igbt_output_turnoff --simulator sentaurus --project <USER_OWNED_PROJECT>",
            maturity=RunnerMaturity.REAL_EXTERNAL,
            dimensionality="external_2d_or_3d_transient",
            solver_backend="sentaurus_external_workspace",
            solver_invoked=True,
            signoff_level="external_workspace_tcad_evidence",
            metrics=["on_state_voltage_v", "blocking_voltage_v", "tail_current_a", "turnoff_energy_j"],
            artifacts=["state.json", "runner_contract.json", "sentaurus_state.json", "solver_logs", "curve_csv", "deck_ir_artifacts"],
            capability_boundary="Runs only against a user-owned Sentaurus IGBT transient workspace/profile outside the repository.",
            signoff_gaps=["user_owned_project_required", "license_profile_required", "lifetime_thermal_calibration", "transient_convergence", "golden_or_measured_correlation"],
            public_source_ids=["sentaurus_quasistationary_training", "genius_tcad_open", "gts_tutorial_catalog"],
        ),
        IndustrialRunnerDescriptor(
