...
class ExtendedDeviceSweepTest(unittest.TestCase):
    ...
    def test_power_mosfet_physics_fidelity_has_avalanche_evidence(self) -> None:
        ...
        self.assertEqual(metrics["solver_backend"], "devsim_1d_power_mos_drift_poisson_avalanche")
        self.assertGreaterEqual(metrics["devsim_bias_solved_points"], 1)
        self.assertGreaterEqual(metrics["devsim_bias_failed_points"], 0)
        ...
        self.assertEqual(state.tcad_deck_spec["physics_models"]["coupling_status"], "equation_coupled")
        self.assertEqual(metrics["source_doping_cm3"], 1.0e19)
        self.assertEqual(metrics["drain_doping_cm3"], 1.0e19)
        self.assertEqual(state.tcad_deck_spec["doping"]["source_doping_cm3"], 1.0e19)
        self.assertEqual(state.tcad_deck_spec["doping"]["drain_doping_cm3"], 1.0e19)
        ...
        self.assertTrue(Path(state.final_summary["artifacts"]["runner_contract"]).exists())
        self.assertTrue(Path(state.final_summary["artifacts"]["devsim_summary"]).exists())
        ...
        self.assertNotIn("structured_tcad_spec", benchmark.summary["signoff_evidence_pack"]["missing_evidence"])
        codes = {check.code for check in benchmark.checks}
        self.assertIn("power_mos_devsim_solver_invoked", codes)
